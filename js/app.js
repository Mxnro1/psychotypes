...
function openSection(key) {
  ...
  // Animate child form elements with a small stagger for nicer entrance
  try { animateSectionEntries(key); } catch (e) { /* ignore */ }
}
...
// Start button: hide welcome, show cards with animation
function startFromWelcome() {
  ...
  // Fade out welcome
  welcome.style.animation = "fadeOutScale .4s cubic-bezier(.2,.9,.3,1) forwards";
  
  // Show grid with staggered card animations
  setTimeout(() => {
...
// Helper animations (if not already in CSS)
...
// Animate labels/selects/buttons inside a section with staggered delays
function animateSectionEntries(key) {
  const sec = document.getElementById('section-' + key);
  if (!sec) return;

  // Elements to animate: headings, muted text, labels, selects, inputs, buttons, back button
  let selector = 'h2, .muted, label, select, input[type=text], button, .back-btn';

  // For the full test block include question text and score buttons specifically
  if (key === 'test') {
    selector += ', .question, .question > div:first-child, .question .scale-btn';
  }

  const items = Array.from(sec.querySelectorAll(selector));
  items.forEach((el, idx) => {
    // mark as animated-item (CSS handles initial state)
    el.classList.remove('enter');
    el.classList.add('animated-item');
    // smaller stagger for test section
    const baseDelay = (key === 'test') ? 40 : 80;
    const delayMs = idx * baseDelay;
    el.style.animationDelay = `${delayMs}ms`;
    // if test, make animation slightly faster
    if (key === 'test') el.style.animationDuration = '320ms';
...
function showTypeCombo() {
  ...
  // prepend animated pills (reuse traitPop animation via trait-item.enter)
  const coreObj = getRadical(main) || { name: '-' };
  const shellObj = getRadical(second) || { name: '-' };
  const markObj = getRadical(third) || { name: '-' };

  const pillsHtml = `<div style="display:flex; gap:8px; align-items:center; margin-bottom:10px;">
      <div class="pill trait-item radical-${main}">${coreObj.name}</div>
      <div class="pill trait-item radical-${second}">${shellObj.name}</div>
      <div class="pill trait-item radical-${third}">${markObj.name}</div>
    </div>`;

  box.innerHTML = pillsHtml + box.innerHTML;

  // stagger-enter the pills to match the interpretation block animation
  const pills = box.querySelectorAll('.pill.trait-item');
  pills.forEach((el, idx) => {
    const delayMs = idx * 90; // 90ms stagger
    el.style.animationDelay = `${delayMs}ms`;
...
function renderFullTestUI() {
  ...
  QUESTIONS_MODEL.questions.forEach((q, i) => {
    // include meta attributes so we can treat some questions specially (punctuality/lie)
    const metaAttr = q.meta ? `data-meta="${q.meta}"` : '';
    const metaClass = q.meta ? ` ${q.meta}-question` : '';
    html += `
      <div class="question${metaClass}" ${metaAttr}>
...
function handleFullTestSubmit() {
  ...
  // Compute lie / honesty indicator from questions marked with meta: "lie"
  const lieQuestions = QUESTIONS_MODEL.questions.filter(q => q.meta === 'lie');
  let lieSum = 0, lieMax = 0;
  lieQuestions.forEach(q => {
    const sel = document.querySelector(`input[name="${q.id}"]:checked`);
    if (!sel) return;
    let val = Number(sel.value);
    // if question marked reverse, invert the scale so higher always means more honest
    if (q.reverse) val = 6 - val; // 1->5, 5->1
    lieSum += val;
    lieMax += 5;
  });
  const honestyPercent = lieMax > 0 ? Math.round((lieSum / lieMax) * 100) : null;
  ...
  box.innerHTML = `
    ...
    ${honestyPercent !== null ? `<p><strong>Индикатор честности (самоотчёт):</strong> ${honestyPercent}%</p>` : ''}
    <hr>
...
function renderTraitsUI() {
  ...
  Object.entries(TRAITS_BY_RADICAL).forEach(([id, data]) => {
    ...
    data.traits.forEach(trait => {
      ...
      // add class 'trait-item' for staggered animation and data-order
      html += `
        <label class="trait-item" data-order="${order}" style="display:flex; align-items:center; gap:6px; cursor:pointer; font-size:12px;">
  ...
  // Stagger enter animation for trait items
  const items = container.querySelectorAll('label.trait-item');
  items.forEach((el, idx) => {
    const delay = (el.dataset.order ? Number(el.dataset.order) : idx) * 60; // ms
    el.style.animationDelay = `${delay}ms`;
    // trigger the animation class slightly after insertion
    setTimeout(() => el.classList.add('enter'), 20 + delay);
  });
}
...
function computeTraitsInterpretation() {
  ...
  box.innerHTML = `
    ...
    <div style="display:flex; gap:8px; align-items:center; margin-bottom:8px;">
      <div class="pill radical-${coreId}">${coreObj.name}</div>
      <div class="pill radical-${shellId}">${shellObj.name}</div>
      <div class="pill radical-${markId}">${markObj.name}</div>
